...
\begin{example}
A cannon fires a $10$ kg cannonball with initial speed $20$ m/s at an angle of $20^\circ$ above the horizontal from a point $50$ metres above the ground.
...
Finally, state what the actual distance from the cannon to the landing point is. Hint: use Pythagoras' theorem.
...
To find the actual distance, we use that the vertical distance travelled is $50$ metres along with the horizontal distance $74.6$ metres. This forms a right angled triangle so we can use Pythagoras' theorem to find the actual distance $d$ from the cannon:
\[
...
\begin{example}
A cannon fires a $10$ kg cannonball with initial speed $20$ m/s at an angle of $\theta$ to the horizontal from ground level.
...
At the same instant, the hare is moving in the same direction with speed $10$ m/s and constant deceleration $2$ m/s$^2$.
