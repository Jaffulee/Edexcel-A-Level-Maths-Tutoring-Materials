...
To get the overall length of the vector, we can now ask ourselves how we can find the length of the line formed by drawing the shape formed by going in each axis' direction as instructed by the vector.
...
This gives us the equations for lengths of two and three-dimensional vectors as the square root of the sum of squares of the components! This should now be clear that it is the same as Pythagoras.
...
\begin{solution}
    ...
    \begin{enumerate}
        ...
        \item Reversing the original route: $\vec{CA} = -\vec{BC} - \vec{AB}$
        \item Whatever this route is: $\vec{CA} = -\vec{OC} + \vec{OB} + \vec{BA}$
        \item \dots
    \end{enumerate}
    In fact, the right-hand side will be identical for all of these options! Let's calculate the first two:
...
\begin{example}
    ...
    $N$ is at the midpoint of $D$ and $E$, and $M$ is at the midpoint of $B$ and $G$ as in the diagram below.
...
\begin{solution}
    ...
    Now consider $\vec{BM}$. Since it is halfway along $\vec{BG}$, we know that 
    ...
    Similarly, since $\vec{DN}$ is halfway along $\vec{DE}$, we can calculate 
