...
\begin{solution}
    ...
    Substituting $h(x)$ into this equation gives
...
This function is defined everywhere except for $x=0$, where we would be dividing by $0$. Therefore its (maximal) domain is every number except for $0$. 
...
\end{example}
\begin{solution}
    We know that this function is a straight line that is increasing, so therefore the range has to be between whatever the leftmost point and the rightmost point is. The place to be careful is that 
...
A lot of calculators can plot functions, and you can use this to help build intuition with what values the function can be, but is not sufficient for an exam. For the exam, you really need to check a few things. 
On a domain, the function's minimum and maximum values come from\footnote{More precisely, the function might not reach the minimum or maximum value, but just approach it arbitrarily closely, which is a distinction we will not worry about for these A-level notes.} one of the following special cases:

\begin{enumerate}
    \item The endpoints of the domain (if they exist)
    \item The points where the function has a turning point (the derivative is $0$ and it is flat at that point), or a sharp point in the case of a non-smooth function
    \item At any point where you would be dividing by $0$ (or any other undefined point)
\end{enumerate}

This means that to find the range given a domain, the best approach is to:

\begin{enumerate}
    \item First plot the function on your calculator to know roughly where the range is, making note if the minimum or maximum is at one of the above special points
    \item Calculate the value of the function at each of the special points (or if the function gets infinitely big at those points). This requires differentiating to find the turning points, and potentially showing that the turning point is a minimum or a maximum\footnote{You can do this by checking the sign of the second derivative, or by checking the value at each turning point and then deducing that it is the overall minimum or maximum by exhaustion.} 
    \item Write the range based on the above calculations, which you have double checked using the plot in the first step. This can help you a lot when writing exactly the order of each part
\end{enumerate}

\begin{example}
    Find the range of $f(x) = x^2 + 2x + 1$ for the domain $-1.5 \leq x \leq 1$.
\end{example}
\begin{solution}
    If you plot this on a calculator, you would see that the minimum is at the turning point, and the maximum is at the far right of the domain. This is not enough for an exam, so let's calculate the special points and write the answer.

    First calculate the function at the endpoints of the domain:

    \[f(1) = 1^2 + 2(1) + 1 = 4\]
    \[f(-1.5) = (-1.5)^2 + 2(-1.5) + 1 = 0.25\]

    Next, find the turning point by differentiating:
    \[f'(x) = 2x + 2\]
    Set the derivative to zero to find the turning point:
    \[2x + 2 = 0 \therefore x = -1\]
    Calculate the function value at the turning point:
    \[f(-1) = (-1)^2 + 2(-1) + 1 = 0\]

    Therefore, the range of the function is $0 \leq f(x) \leq 4$.
    \qed
\end{solution}

\begin{example}
    Find the range of $f(x) = \frac{1}{x+1}$ for $-1 < x \leq 1$.
\end{example}
\begin{solution}
    If you plot this on a calculator, you would see that the function gets infinitely big as $x$ gets close to $-1$, and the minimum is at the far right of the domain. This is not enough for an exam, so let's calculate the special points and write the answer.

    First calculate the function at the endpoints of the domain:

    \[f(1) = \frac{1}{1+1} = \frac{1}{2}\]
    If we were to try to plug in $-1$, we would be dividing by zero, which is undefined. In fact, in this case it gets larger and larger as we get closer to $-1$\footnote{You can convince yourself of this since $\frac{1}{\left(\frac{1}{2}\right)} = 2$ and  $\frac{1}{\left(\frac{1}{4}\right)} = 4$, and if we keep making the denominator smaller, this value keeps increasing.}.
    Therefore, the function gets infinitely big as $x$ gets close to $-1$.

    There are no turning points in this function, which you could check by differentiating and trying to set it to zero.

    Therefore, the range of the function is $\frac{1}{2} \leq f(x) < \infty$, or just $f(x) \geq \frac{1}{2}$.
    \qed
\end{solution}
...
When it is on the right of the $y$ axis, $x$ is positive and therefore $f(|x|)$ is unchanged. When it is on the left of the $y$ axis, the $x$ point that we plug into $f(x)$ is shifted to be 
...
From our discussion leading up to Figure \ref{fig:mod-function-for-modulus}, we know that this amounts to reflecting everything below the $x$ axis to above the $x$ axis.
...
And for the last step, we can take the minus sign of the red line sketched for  $|f(|x|)|$. This reflects every number about the $x$ axis. This gives us Figure \ref{fig:minusmodmodx-function-for-modulus}.
\begin{figure}[H]
    \centering
...
\begin{example}
    Given that $f(x)$ has range $-16 < f(x) \leq 5$ and reaches every point in between, what range does $|f(x)|$ have?
\end{example}
\begin{solution}
    ...
    For the lower end, we are told that $f$ reaches every point in between. Therefore the lowest point the modulus can reach is $0$.
...
\begin{solution}
    ...
    Therefore, when $-2\leq x \leq 1$, we have that $-5 \leq f(x) \leq 1$ is the range. This completes the first part of the question.
        \item
    ...
    Notice that the function cuts off on the left side. This is because we are only allowed to use $f(x)$ when $x$ is between $-2$ and $1$ from the question. But if $x$ is less than $-1$, then $|x|$ is more than $1$, which is not allowed to be input into the function.
