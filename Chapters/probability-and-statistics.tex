...
In your exam cheat sheet, you get told a lot of values for probabilties of the form $P(X > a)$, where $X$ comes from a specific normal distribution called the 
...
Often times, the question asks you about the mean of the normal distribution, and in particular, how you might model the mean itself.

Given a set of samples $x_1, \dots, x_n$, we can find the sample mean via:

\[ \frac{x_1 + \dots + x_n}{n} \]

If we consider a set of $n$ random variables, $X_1, \dots, X_n$, where each are independently distributed with the same normal distribution $N(\mu, \sigma^2)$, 
...
\begin{example}
    A baby's weight at birth can be modelled as a normal distribution with mean $3.5kg$ and standard deviation $0.5$. 

    Your sibling tells you that a healthy range of baby weights is between $3.37kg$ and $3.76kg$.
    ...
    Using the closest values from the above table, find the probability that the weight of the baby is between $3.37kg$ and $3.76kg$.
\end{example}
...
\begin{solution}
    ...
    The closest values we have from the table are P(Z>0.2533) and P(Z>0.5244), which we can use to find that the probability of the baby weight being between $3.37kg$ and $3.76kg$ is approximately 
...
This means for the exam, it is best practise to convert to the standard normal distribution, and only then use your calculator if you have one. Otherwise, you must use the lookup table provided in the formula booklet.
...
\begin{example}
    ...
    Using the formula book, what is the probability there are no more than $1$ defective chairs in a batch of $50$ chairs, to four significant figures?
\end{example}
\begin{solution}
    ...
    Therefore the probability that there are no more than $1$ defective chairs in a batch of $50$ is $28.94\%$
    \qed
\end{solution}
...
\begin{exercise}
    Analysis shows that $30\%$ of the time, a randomly chosen cereal ball from a cereal manafacturer is chocolate. A box of cereal contains $100$ cereal balls. Using an appropriate choice of distribtution, what is the probability of getting exactly $30$ chocolate balls in the cereal box?
...
\begin{solution}
    ...
    This is accounts for a quarter of all possibilities, which might be more than expected considering the previous number.
...
More rigoroously, it is the maximum probability (in the case of one tail) we are allowing for an event to occur in the extreme of our null hypothesis. For a two-tailed tests, we consider it as two one-tailed tests with half the allowed maximum probability.
...
\begin{solution}
    ...
    we should use a binomial distribution $X \sim B(50,0.5)$, where $X$ is the number of chocolate balls. We want to test if the probability $p$ really is $0.5$, therefore since we are using a two-tailed test, our hypothesis are as follows:
...
\begin{solution}
    ...
    If $X$ is the number of non-orange haired people, we will model $X \sim B(50,0.25)$, and our hypothesis are that either James is right or the number of non-orange haired people have increased, letting p be the probability a person has non-orange hair:
    ...
    James found that $29$ out of his $50$ people had orange hair, meaning that $21$ people had non-orange hair. Since this is within the critical region, there is evidence to reject the the null hypothesis that $75\%$ of people have orange hair, in favour of there less than $75 \%$ of people have orange hair, to this level of significance.
    \qed
\end{solution}
