...
The goal of the trapezium rule is to estimate the area under the curve as in the shaded area of Figure \ref{fig:shaded-function}, i.e. a way to estimate an integral, using only knowledge of the function we are trying to integrate at a few points.
...
\begin{example}
    By splitting the interval between $0$ and $3$ with $4$ equally spaced segments, use the trapezium rule to estimate the integral:
...
\begin{solution}
    The $4$ equally spaced segments will have $h = \frac{3 - 0}{4-1} = \frac{3}{3} = 1$. That means we have:
    \begin{align*}
        x_0 &= 0 \\
        x_1 &= x_0 + h = 1 \\
        x_2 &= x_1 + h = 2 \\
...
\begin{example}
    ...
    By using the Trapezium rule, estimate the area under the curve between $x=-1$ and $x=2.75$.
...
\begin{solution}
    ...
    \begin{align*}
    \int_{x_0}^{x_n}f(x) dx  &\approx  \frac{h}{2} \bigg(  f(x_0) + f(x_n) + 2\big( f(x_{1})+ f(x_{2}) + \dots + f(x_{n-1})\big) \bigg) \\
    \therefore \int_{-1}^{2.75}x^2 dx  &\approx  \frac{1.25}{2} \bigg(  -1 + 5 + 2\big( 4 + 6 \big) \bigg) \\
    &= \frac{1}{2} \bigg(  4+ 2\big( 10 \big) \bigg) \\
    &= \frac{1.25 \times 24}{2}\\
    &= 15
    \end{align*} 
    ...
    Now it becomes clear that the trapeziums are always under the curve. When the curve is below zero, this means that the trapezium area is less than the curve's area. What about when the function 
    ...
    This makes it unclear whether the area in this case is less than or more than the actual function.
    \qed
\end{solution}
